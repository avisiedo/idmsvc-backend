...
COMPOSE_VARS_DATABASE=\
	DATABASE_USER="$(subst ",,$(DATABASE_USER))" \
	DATABASE_PASSWORD="$(subst ",,$(DATABASE_PASSWORD))" \
	DATABASE_NAME="$(subst ",,$(DATABASE_NAME))" \
	DATABASE_HOST="$(subst, ",,$(DATABASE_HOST))" \
	DATABASE_EXTERNAL_PORT="$(subst ",,$(DATABASE_EXTERNAL_PORT))"
...
COMPOSE_VARS_DB_MIGRATE=\
	...
	DATABASE_NAME="$(subst ",,$(DATABASE_NAME))" \
	DATABASE_HOST="$(subst, ",,$(DATABASE_HOST))" \
	DATABASE_PORT="$(subst, ",,$(DATABASE_EXTERNAL_PORT))"

COMPOSE_VARS_KAFKA=\
	KAFKA_CONFIG_DIR="$(subst ",,$(KAFKA_CONFIG_DIR))" \
	KAFKA_DATA_DIR="$(subst ",,$(KAFKA_DATA_DIR))" \
	ZOOKEEPER_CLIENT_PORT="$(subst ",,$(ZOOKEEPER_CLIENT_PORT))" \
	KAFKA_TOPICS="$(subst ",,$(KAFKA_TOPICS))"
...
COMPOSE_VARS_RBAC_MOCK=\
    MOCK_RBAC_CONTAINER="mock-rback"

COMPOSE_VARS= \
	CONTAINER_IMAGE_BASE="$(subst ",,$(CONTAINER_IMAGE_BASE))" \
	CONTAINER_IMAGE_TAG="$(subst ",,$(CONTAINER_IMAGE_TAG))" \
	$(COMPOSE_VARS_DATABASE) \
	$(COMPOSE_VARS_KAFKA) \
	$(COMPOSE_VARS_APP)
